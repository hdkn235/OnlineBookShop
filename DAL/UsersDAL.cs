...
namespace BookShop.DAL
{
	...
	public partial class UsersDAL
	{
		...
		public int Add(BookShop.Model.Users model)
		{
            ...
            strSql.Append("LoginId,LoginPwd,Name,Address,Phone,Mail,UserStateId,ActiveCode)");
            ...
            parameters[7].Value = model.ActiveCode;
		...
		public bool Update(BookShop.Model.Users model)
		{
			...
			parameters[4].Value = model.UserState.Id;
		...
		public BookShop.Model.Users DataRowToModel(DataRow row)
		{
			...
			if (row != null)
			{
				...
				{
                    int userStateId = int.Parse(row["UserStateId"].ToString());
                    model.UserState = new UserStateDAL().GetModel(userStateId);
				}
                if (row["Actived"] != null)
                {
                    model.Actived =Convert.ToBoolean(row["Actived"]);
                }
